...
export abstract class BaseStep extends BaseStepInterface<string> {
     ...
     * @returns 
     */
    run(isForce: boolean = false, refineIndex: number = 0): Observable<string> {
...
export class MultiStep extends BaseStepInterface<string[]> {
    ...
    run(isForce: boolean = false, refineIndex: number = 0): Observable<string[]> {
        ...
        if (this.isSkip) {
            ...
            return forkJoin(this.childStepList.map(step => { step.isSkip = this.isSkip; return step.run(); }));
        } else {
            return new Observable<string[]>((subscriber) => {
                forkJoin(this.childStepList.map(step => step.run(isForce, refineIndex))).pipe(
